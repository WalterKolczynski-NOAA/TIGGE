...
if( -e $primary ) 
	{ unlink($primary); }
print STDOUT (`perl $ENV{TIGGE_TOOLS}/run_ncep_convert.pl $cycleYMDH >&./rerun.$cycleYMDH.log`);

$theDate = scalar(localtime(time));
print STDOUT "$theDate : Concatenating secondary \n ($secondary) $secondarySize bytes\n\n";
