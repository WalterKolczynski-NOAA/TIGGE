...
static int  unpack_double(grib_accessor* a, double* val, size_t *len)
{
  ...
  if ((abitmap=grib_find_accessor(a->parent->h,self->bitmap))!=NULL) {
	  ...
	  bitmap=grib_context_malloc_clear(a->parent->h->context,sizeof(char)*bitmap_len);
...
static long value_count(grib_accessor* a){
  ...
  if( grib_get_long_internal(a->parent->h,self->p1, &p1) != GRIB_SUCCESS)
    return 0;
  if( grib_get_long_internal(a->parent->h,self->extraValues, &extraValues) != GRIB_SUCCESS)
	  return 0;
