...
#	print STDOUT (`$GRIB_LS -p $gribLsKeysString $inFile`);
	print STDOUT "\nSYSOUT : " . system "/usr/local/bin/grib_ls";
    print STDOUT "\nSYSOUT : " .`/bin/ls /home/nomads/tigge`;
...
if( (-e $qcTagFile) && !(-z $qcTagFile) ) 
	...
	return("$headline *-*-PASSED-*-* tagFile exists $qcTagFile:
...
return("$headline *-*-PASSED-*-* ".localtime(time)."\n");
}
...
foreach my $run ( sort @uncompletedRuns ) 
	...
	if(-f $pathToOutTag) 
		{ next; }
	print " DTG $run : Hasn't been completed! \n";
	}
...
for( my $r = 0; $r < $NUMRUNS; $r++ )
 ...
 if( $QC eq "yes" )
    ...
    print STDOUT (`perl $ENV{'TIGGE_TOOLS'}/run_ncep_convert.pl $uncompletedRuns[$r] `);

    print STDOUT (" $ENV{'TIGGE_TOOLS'}/ncdcTigge $uncompletedRuns[$r] all
	...
	if( $qcOutputResults[0] eq "PASS" )
		...
		print "$0 : QC Passed.  Sending output to its archive directory...\n\n";
	...
	print STDOUT "$headline Completed cycle $latestRun!\n\n\n";
    print STDOUT "\n ===---===---===---===---===---===---===---===\n\n";
    }
...
print STDOUT (`$ENV{'TIGGE_TOOLS'}/runUploader.pl`);
...
my $tarzFile1         = "${archiveDir}/gens-tigge_3_${cycle}.tar.gz";
...
my @tarRtn =  (`tar --exclude=*gens-ncdc*.grb2 --exclude=*.tar.gz --exclude=*OUTPUT.QC -czf $tarzFile1 ./$cycle/`);
if( @tarRtn ) 
	{
