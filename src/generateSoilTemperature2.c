...
void generateSoilTemperature(int yyyy, int mm, int dd, int hh, int fff, int em)
	...
	*/
	...
	// first, are the files here?
	if(aFile == NULL)
		{
		fprintf(stderr, 
			"Error, could not read the required a file.  Exiting generateSoilTemperature()\n");
		return;
		}
	...
	if( bFile == NULL )
		{
		fprintf( stderr,
			"Error, could not read the required b file.  Exiting generateSoilTemperature() \n");
		return;
		}
