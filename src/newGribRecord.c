...
**
*/
grib_handle* newGribRecord(int yyyy, int mm, int dd, int hh, int fff, int em, int n_lat, int n_lon){
	grib_handle *h = NULL;
	size_t size = n_lat * n_lon;
	...
	/* grid information */
    GRIB_CHECK(grib_set_long(h,"Ni",n_lon),0);
    GRIB_CHECK(grib_set_long(h,"Nj",n_lat),0);
    ...
    See grib2/3.3.table */
    long lat_inc = 180000000/(n_lat-1);
    long lon_inc = 360000000/n_lon;
    GRIB_CHECK(grib_set_long(h,"resolutionAndComponentFlags",48),0);
    GRIB_CHECK(grib_set_long(h,"latitudeOfLastGridPoint",-90000000),0);
    GRIB_CHECK(grib_set_long(h,"longitudeOfLastGridPoint",(360000000-lon_inc)),0);
    GRIB_CHECK(grib_set_long(h,"iDirectionIncrement",lon_inc),0);
    GRIB_CHECK(grib_set_long(h,"jDirectionIncrement",lat_inc),0);
    ...
    GRIB_CHECK(grib_set_long(h,"generatingProcessIdentifier",get_gen_proc_id(yyyy,mm,dd,hh)),0);
    ...
    GRIB_CHECK(grib_set_long(h,"numberOfForecastsInEnsemble", get_n_members(yyyy,mm,dd,hh)),0);

    /* grib 2 Section 5 DATA REPRESENTATION SECTION */
    GRIB_CHECK(grib_set_long(h,"numberOfValues",size),0);
...
grib_handle* newBlankGribRecord( long discipline, long category, long parmNumber, long firstLevelType, int n_lat, int n_lon)
	{
	grib_handle *tmp;
    double* v = NULL;
	int i = 0;
    size_t size = n_lat * n_lon;
	tmp = newGribRecord( 0,0,0,0,0,0,n_lat,n_lon );
	...
	// size_t size = get_n_points(yyyy,mm,dd,hh);
	v = (double*)calloc(size,sizeof(double));
	if(!v) {
