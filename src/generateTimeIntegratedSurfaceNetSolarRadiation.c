...
void** loadDataForTimeIntegratedSurfaceNetSolarRadiation(void** gribBuffers){
	...
	while(!found1 || !found2){
		...
		// search for the first one.
			...
			if (h1 == NULL) {
				printf("Error (generateTimeIntegratedSurfaceNetSolarRadiation.h1) unable to create handle from file. error code: %d \n", err);
				return NULL;
			}
		...
		// search for the second one.
			...
			if (h2 == NULL) {
				printf("Error (generateTimeIntegratedSurfaceNetSolarRadiation.h2) unable to create handle from file. error code: %d \n", err);
				return NULL;
			}
