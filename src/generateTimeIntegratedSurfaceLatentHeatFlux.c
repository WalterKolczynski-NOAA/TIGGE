/* generate Time-Integrated surface latent heat flux
**
**  Generates a valid tigge file for Time-Integrated surface latent heat flux
**
** Steven Anthony, 2007
**
*/
...
void generateTimeIntegratedSurfaceLatentHeatFlux(int yyyy, int mm, int dd, int hh, int em, void** resource){
	int fff = 0;
	int i = 0;
	...
	for(fff = 0; fff<=FORECAST_HOURS; fff+=6){
		...
		ncepGribRecord = grib_handle_new_from_message(NULL, resource[fff/6], bytesPerRecord[TIME_INTEGRATED_SURFACE_LATENT_HEAT_FLUX][fff/6]);
		...
		generateFileName(TIME_INTEGRATED_SURFACE_LATENT_HEAT_FLUX, yyyy, mm, dd, hh, fff, em, fileName);
		...
		if(fff == 0){
			...
			for(i=0; i<ncepValuesSize; i++){
				...
				//          "positive downward radiation" TIGGE convention (-21600)
				tiggeTimeIntegratedSurfaceLatentHeatFluxValues[i] = runningSum[i] * -21600.0;
			}
		}
		...
		GRIB_CHECK(grib_set_long(gh, "parameterNumber", 10), 0);
		...
		free(ncepValues);
		free(tiggeTimeIntegratedSurfaceLatentHeatFluxValues);
...
/*
** loadDataForTimeIntegratedSurfaceLatentHeatFlux()
**  Loads the required data needed to calculate the slhf variable.
**
** Parameters:
...
void** loadDataForTimeIntegratedSurfaceLatentHeatFlux(void** gribBuffers, int n_lat, int n_lon){
	...
	if(!numberOfSavedRecords[TIME_INTEGRATED_SURFACE_LATENT_HEAT_FLUX]){
		numberOfSavedRecords[TIME_INTEGRATED_SURFACE_LATENT_HEAT_FLUX] = 0;
	}
	
	// seek to the correct grib_handle we need
	ncepDiscipline = ncepCategory = ncepVariable = ncepLevel = -1; 
	while( (ncepDiscipline != 0) || (ncepCategory != 0) || (ncepVariable != 10) || (ncepLevel != 1) ){
		if(h != NULL){
			grib_handle_delete(h);
                ...
                if (h == NULL && ignoreMissingInput == 0 )
                        {
                        printf("Error (generateTimeIntegratedSurfaceLatentHeatFlux) unable to create handle from file. error code: %d \n", err);
                        return(NULL);
                        }
	...
	retBuffer = addGribRecordToArray(TIME_INTEGRATED_SURFACE_LATENT_HEAT_FLUX, gribBuffers, buffer, size);
