...
while( ($dtg lt $endDate) && ($breaker < 10000) ) 
	...
	my $ageCheck  = -M $pathCheck;
		# This avoid false positives from output that is in the process of being built.
	if( $ageCheck && $ageCheck < 0.1 ) 
		{ print STDOUT "$pathCheck is too recent, skipping.\n";  next; }
