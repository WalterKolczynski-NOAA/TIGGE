...
my $useForce = 0;
if( scalar grep(/\-f/,@ARGV) )   { $useForce = 1; }
...
while( $looper <= $rectifyNow ) 
	...
	print STDOUT " >> $thisCycle ";
		# this does the scan.
	my $tmp0 = ncdcTigge::ncepNomadsBackupSource($thisCycle,$useForce);
	...
	if( defined($tmp[2]) ) 
		...
		{
	#	$recoverNum += int(ncdcTigge::ncepNomadsBackupSource($thisCycle,$useForce));
